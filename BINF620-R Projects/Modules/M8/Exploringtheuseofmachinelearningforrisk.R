#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6402678/pdf/pone.0213258.pdf
...
### These codes compute standard and penalized regressoin models using 4 years of data from 2009 to 2012 to predict 2013 health care costs  ###
...
### test standard linear regression on the full set of 2009-2012 predictors

load("trainset_fullset_4yrs.Rdata")
load("testset_fullset_4yrs.Rdata")

lmfit<-lm(total_allowed_2013 ~ ., data = trainset_fullset_4yrs)
preds<-predict(lmfit, newdata = testset_fullset_4yrs)
df_preds<-as.data.frame(preds)
df_actual<-as.data.frame(testset_fullset_4yrs[1])%>%rename(actual=total_allowed_2013)
performance(df_preds, df_actual)


### Penalized regression of health care costs on 4 years of prior data

# Lasso regression
y<-trainset_fullset_4yrs$total_allowed_2013
x<-as.matrix(trainset_fullset_4yrs[-1])
nx<-as.matrix(testset_fullset_4yrs[-1])
...
### test standard linear regerssion on 2009-2012 predictors selected by lasso regression with lambda.min

load("trainset_reducedset_4yrs.Rdata")
load("testset_reducedset_4yrs.Rdata")

lmfit_reducedset_4yrs<-lm(total_allowed_2013 ~ ., data = trainset_reducedset_4yrs)
preds<-predict(lmfit_reducedset_4yrs, newdata = testset_reducedset_4yrs)
df_preds<-as.data.frame(preds)
performance(df_preds, df_actual)
