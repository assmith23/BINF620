...
```{r split, eval=TRUE, message=FALSE, warning=FALSE, include=}
n <- nrow(modelData)
...
```{r logisticReg, eval = TRUE, include = FALSE}
# Logistic Regression
...
```{r neuralNet, eval = TRUE, include = FALSE}
# Final Model
n <- nrow(modelData)
train_size <- floor(0.7 * n)
train_indices <- sample(seq_len(n), train_size)
...
## KMeans
Further analysis was performed do to the limited significance of the neural network. KMeans was performed to sort our data into 2 groups work towards the groups being that we have created for our mental health concerns.
...
# Results
## Model Used
```{r neuralNet_results, eval = TRUE, include = FALSE}
