...
scaled_data <- as.data.frame(scale(pima_raw[, -9]))
scaled_data$pedigree <- pima_raw$pedigree
```
...
fit_neural_network <- function(train_data, test_data, size) {
  # Prepare scaled predictors
  train_scaled <- as.data.frame(scale(train_data[, c("pregnant", "glucose", "pressure", 
                                                     "triceps", "insulin", "mass", "age")]))
  train_scaled$pedigree <- train_data$pedigree
  
  test_scaled <- as.data.frame(scale(test_data[, c("pregnant", "glucose", "pressure", 
                                                   "triceps", "insulin", "mass", "age")]))
  test_scaled$pedigree <- test_data$pedigree
